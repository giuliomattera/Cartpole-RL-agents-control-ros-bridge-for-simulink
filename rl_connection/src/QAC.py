...
class QAC():
    ...
    def Actor(self):
        ...
        return tf.keras.Model(input, [mu, std])
    ...
    def Critic(self):
        ...
        batch = tf.keras.layers.BatchNormalization()(state_input)

        state_out_critic = tf.keras.layers.Dense(32, 
        activation=tf.keras.layers.LeakyReLU(alpha=0.1),
         kernel_initializer= initializer)(batch)

        # Action as input
        action_input = tf.keras.layers.Input(shape=(self.num_actions))

        action_out_critic = tf.keras.layers.Dense(32,
        activation=tf.keras.layers.LeakyReLU(alpha=0.1),
        ...
        concat = tf.keras.layers.Dense(16,
         activation=tf.keras.layers.LeakyReLU(alpha=0.1), 
        kernel_initializer= initializer,
        kernel_regularizer=tf.keras.regularizers.l1_l2(l1=1e-4, l2=1e-4))(concat)
    ...
    def update(self, state, action, reward, next_state, next_action, batch):
        ...
        with tf.GradientTape() as gradient:
            ...
            prob = tfp.distributions.Normal(mu, std)

            log_prob = prob.log_prob(action)
            
            critic_q = critic_model([state, action], training=True)

            actor_loss = tf.math.reduce_mean(-log_prob*(reward-critic_q))

            if np.isnan(actor_loss):
                actor_loss = tf.Tensor(0, dtype = tf.float32)
        ...
        return TD_error, log_prob*(reward-critic_q)
...
def agent_out(agent, actor_model, state):
    ...
    a0 = np.float32(a0)*agents.scale_effort
...
def train(total_trajectory, avg_TD, TRAIN = True):
    ...
    global episode, EARLY, s0, a0, r, G_t, al
    ...
    if TRAIN == True:
        if num_sample > 2:
            ...
            if num_sample < BATCH:
                ...
                print('The avg Temporal difference error : ', avg_TD)
                print('Avg Actor loss : ', al)

            episode = episode + 1
            
            with train_summary_writer.as_default():
                tf.summary.scalar('Average of TD error', avg_TD, step=episode)
                tf.summary.scalar('Actor loss',  al/num_sample, step=episode)
            ...
            print('Training is skipped because we have only few recording from envs :  ', t)
...
def communicate(agents, actor_model):
    ...
    global i, t, states, actions, traj, G_t, r, a0, s0, action
    ...
    if i < 2:
        ...
        G_t = r*agents.gamma**t + G_t #update discounted reward for metrics
...
if __name__ == '__main__':
    try:
        ...
        states, actions, traj, total_trajectory, avg_TD, al = [], [], [], [], 0, 0
